...
std::vector<std::unique_ptr<Room>> RoomGenerator::generateMap(int nb_rooms) 
{
    ...
    std::map<std::pair<int, int>, Room*> position_to_room;
    ...
    while (cur_index < nb_rooms) {
        ...
        if (roomPositions.contains(std::pair(new_x, new_y))) {
            auto room = position_to_room[std::pair(new_x, new_y)];
            room->open_path(rooms[it->second]->get_position());
            rooms[it->second]->open_path(room->get_position());
            room->linkToRoom(rooms[it->second].get());
            rooms[it->second]->linkToRoom(room);
        ...
        position_to_room[std::pair(new_x, new_y)] = new_room.get();
